...
async def test_agent_selector_compatibility(
    ...
    planner_agent = await registry.get("planner")
    await manager.register(ManagerAgentAdapter(planner_agent, capabilities=frozenset({"planning"})))
    coder_agent = await registry.get("coder")
    await manager.register(ManagerAgentAdapter(coder_agent, capabilities=frozenset({"planning"})))
